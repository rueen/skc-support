      ...
      <div class="table-header">
        <div class="left">
          <a-form layout="inline" :model="searchForm">
            <a-form-item :label="$t('messages.title')">
              <a-input
            ...
            <a-form-item :label="$t('messages.status')">
              <a-select
              ...
              >
                <a-select-option 
                  v-for="option in messagesStatusOptions"
                  :key="option.value" 
                  :value="option.value"
                >
                  {{ option.text }}
...
const messagesStatusOptions = computed(() => {
  // 如果枚举数据还未加载完成，则返回空数组
  if (!enumStore.loaded) {
    return []
  }
  ...
  // 使用store提供的方法获取选项列表
  return enumStore.getEnumOptions('MessagesStatus')
})
...
const columns = computed(() => [
  ...
  },
  {
    ...
    customRender: ({ text }) => {
      // 使用store提供的方法获取枚举文本
      return enumStore.getEnumText('MessagesStatus', text)
    }
  },
  {
...
const handleDelete = async (record) => {
  try {
    const res = await del('messages.delete', {
      id: record.id
    }, {
      urlParams: {
        id: record.id
...
const handleEdit = (record) => {
  router.push(`/messages/edit/${record.id}`)
}
